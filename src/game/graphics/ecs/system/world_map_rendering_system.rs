...
impl System for WorldMapRenderingSystem {
        ...
        match camera_option {
            ...
            Some(camera) => {
                ...
                for tilemap_entity_id in accessor.borrow_ids::<TilemapComponent>(manager).unwrap() {
                    ...
                    renderer.render_tilemap_2d(tilemap, self.rendering_state.borrow().deref(), camera)
                }
            }
        };
    }
